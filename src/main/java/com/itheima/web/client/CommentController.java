...
public class CommentController {
    ...
    public String liuyan(@RequestParam(value = "page", defaultValue = "1") int page,
                         @RequestParam(value = "count", defaultValue = "10") int count,
                         HttpServletRequest request) {
        PageInfo<Comment> pageInfoteo = commentServcieImpl.selectArticleWithPage(page,count);
        request.setAttribute("articless", pageInfoteo);
        return "back/liuYan";
    }
    // 文章删除
    @PostMapping(value = "/liuyanDelete")
    @ResponseBody
    public ArticleResponseData liuyanDelete(@RequestParam Integer id) {
        try {
            commentServcieImpl.deleteArticleWithId(id);
            logger.info("评论删除成功");
            return ArticleResponseData.ok();
        } catch (Exception e) {
